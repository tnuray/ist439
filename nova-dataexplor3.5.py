...
from sklearn.metrics import confusion_matrix #yeni eklendi confusion matrix için
...
class DataExplor(ctk.CTk):
    ...
    def load_sample(self, dataset):
        try:
            ...
            self.text_display.insert("end",
                                     f"\n\n--- Label Distribution (Sample Data) ---\n")
            self.text_display.insert("end",
                                     df[label_col].value_counts().sort_index().rename("").to_string())
            self.text_display.insert("end",
...
# Yelp Llm  prediction part
    def run_llm_prediction(self):
        ...
        try:
            ...
            # Seed eklendi - reproducibility için
            np.random.seed(42)
            ...
            test_df = pd.read_csv(test_file)
            test_df['sentiment'] = full_labeled_df['sentiment'].values
            ...
            self.current_df = test_df
            self.current_filename = "yelp_predictions"
            # confusion matrix son eklenen kod (366-377 arası)
            ...
            model_accuracy = (tp + tn) / total_support
            ...
            # 5. Confusion Matrix ekrana yaz
            self.text_display.insert("end", " CONFUSION MATRIX:\n")
            self.text_display.insert("end", "                [ Predicted: n ]   [ Predicted: p ]\n")
            self.text_display.insert("end", f"Actual n:       {tn:<5}              {fp:<5}      (Total: {tn + fp})\n")
            self.text_display.insert("end", f"Actual p:       {fn:<5}              {tp:<5}      (Total: {fn + tp})\n")
            self.text_display.insert("end", "-" * 65 + "\n\n")

            # 6. Metrikler ekrana yaz
            self.text_display.insert("end", "Model Efficiency (Overall Results):\n")
            self.text_display.insert("end", "-" * 65 + "\n")
            self.text_display.insert("end", f"{'Model Accuracy:':<32} {model_accuracy * 100:.2f}%\n")
            self.text_display.insert("end", f"{'Model Precision:':<32} {model_precision * 100:.2f}%\n")
            self.text_display.insert("end", f"{'Model Recall:':<32} {model_recall * 100:.2f}%\n")
            self.text_display.insert("end", f"{'Model F1-Score:':<32} {model_f1 * 100:.2f}%\n")
            self.text_display.insert("end", "-" * 65 + "\n")
            self.text_display.insert("end", f"{'Total Support:':<32} {total_support}\n\n")
            # Render Project Metadata Footers
            self.text_display.insert("end", f" ℹ️  Training Source: yelp_400_labeled_final1.csv\n")
            self.text_display.insert("end", " ℹ️  Method: Supervised Learning (Trained by Nuray's Labels)\n")
            self.text_display.insert("end", "-" * 65 + "\n\n")
  ...
  #Ag news Llm prediction part
    def run_ag_news_prediction(self):
        ...
        try:
            ...
            # confusion matrix son eklenen kod (463-474 arası)
            cm = confusion_matrix(test_df['sentiment'], predictions)
            tn = cm[0, 0]
            fp = cm[0, 1]
            fn = cm[1, 0]
            ...
            # Render Project Metadata Footers
            self.text_display.insert("end", f" ℹ️  Training Source: ag_news_383_labeled.csv\n")
            self.text_display.insert("end", " ℹ️  Method: Supervised Learning (Trained by Nuray's Labels)\n")
            self.text_display.insert("end", "-" * 65 + "\n\n")
...
#imdb llm prediction part
    def run_imdb_prediction(self):
        ...
        try:
            ...
            np.random.seed(42)  # Seed eklendi - reproducibility için
            test_df['sentiment'] = full_labeled_df['sentiment'].values  #eklendi
            predictions = model.predict(test_df['text'].astype(str))
            ...
            #confusion matrix son eklenen kod (558-569 arası)
            cm = confusion_matrix(test_df['sentiment'], predictions)
            tn = cm[0, 0]
            fp = cm[0, 1]
            fn = cm[1, 0]
            ...
            self.text_display.insert("end", f" ℹ️  Training Source: {labeled_file}\n")
            self.text_display.insert("end", " ℹ️  Method: Supervised Learning (Trained by Nuray's Labels)\n")
            self.text_display.insert("end", "-" * 65 + "\n\n")
